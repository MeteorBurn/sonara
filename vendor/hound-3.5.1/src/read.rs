...
pub fn read_until_data<R: io::Read>(mut reader: R) -> Result<(WavSpecEx, u32)> {
    ...
    loop {
        ...
        match header.kind {
            ...
            ChunkKind::Unknown => {
                // RIFF chunks are word-aligned, so odd payloads have one pad byte.
                let padded_len = header.len as usize + (header.len as usize & 1);
                try!(reader.skip_bytes(padded_len));
            }
        }
